...
class MemberController extends Controller
{
    ...
    public function member_login(Request $request)
    {
        if( Auth::guard('member')->check() )
        {
            ...
            // if (Auth::guard('member')->attempt(['email' => $request->email, 'password' =>password_verify('password', $request->password)], $request->remember)) {
    ...
    public function member_care(Request $request)
    {
        ...
        if( Auth::guard('member')->check() )
        {
            ...
            $members = Member_Relation::where(['member_id'=> $member_id, 'care_list'=> '1'])->orderBy('id', 'desc')->paginate(16);
        ...
        //    }
            ...
            toastr()->error(trans('يرجي تسجيل الدخول اولا حتي تتمكن من رؤية هذة الصفحة'));
            return redirect()->back();
        }
    ...
    public function my_block_list(Request $request)
    {
        ...
        if( Auth::guard('member')->check() )
        {
            ...
            $members = Member_Relation::where(['my_id'=> $auth_id, 'ignore_list'=> '1'])->orderBy('id', 'desc')->paginate(16);
    ...
    public function who_visit_myprofile(Request $request)
    {
        if( Auth::guard('member')->check() )
        {
            ...
            $members = Member_Relation::where(['member_id'=> $auth_id, 'visit_profile'=> '1'])->orderBy('id', 'desc')->paginate(16);
          // dd($members);
            $members_counts = count($members);
