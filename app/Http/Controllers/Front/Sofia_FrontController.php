...
class Sofia_FrontController extends Controller
{
    ...
    {
        $common_questions = Common_Question::orderBy('id', 'desc')->paginate(2);
    ...
    public function front_members_full_filter_search(Request $request)
    {
        if( Auth::guard('member')->check() )
        {
            ...
            })->where('id', '!=', $auth_id)->orderBy('id', 'desc')->paginate(14);
    ...
    public function front_male_members_filter_search()
    {
        if( Auth::guard('member')->check() )
        {
            ...
            $members = Member::where('gender', 'ذكر')->where('id', '!=', $auth_id)->orderBy('id', 'desc')->paginate(14);
            ...
            $members = Member::where('gender', 'ذكر')->orderBy('id', 'desc')->paginate(14);
    ...
    //بحث عن ززوجة معين من خلال الاسم فقط و موجوده في صفحة البحث
    public function front_female_members_filter_search()
    {
        if( Auth::guard('member')->check() )
        {
            ...
            $members = Member::where('gender', 'أنثي')->where('id', '!=', $auth_id)->orderBy('id', 'desc')->paginate(14);
            ...
            $members = Member::where('gender', 'أنثي')->orderBy('id', 'desc')->paginate(14);
    ...
    {
        $stories = Success_Relation::where('status', '0')->orderBy('id', 'desc')->paginate(12);
    ...
    public function latest_members(Request $request)
    {
        ...
        {
            $auth_id = Auth::guard('member')->id();
            $members = Member::where('id', '!=', $auth_id)->where('created_at', '>=', Carbon::now()->subDays(30)->toDateTimeString() )->orderBy('id', 'desc')->paginate(12);

        }else{
            $members = Member::where('created_at', '>=', Carbon::now()->subDays(30)->toDateTimeString() )->orderBy('id', 'desc')->paginate(12);
        }
    ...
    public function latest_females(Request $request)
    {
        ...
        public function health_members_index(Request $request)
        {
            ...
            {
                $auth_id = Auth::guard('member')->id();
                $members = Member::where('id', '!=', $auth_id)->where('health_status', '!=', 'سليم و الحمدلله')->orderBy('id', 'desc')->paginate(12);

            }else{
                $members = Member::where('health_status', '!=', 'سليم و الحمدلله')->orderBy('id', 'desc')->paginate(12);
            }
            ...
            switch($request->submitbutton) {
                ...
                case 'ذكر':
                    if( Auth::guard('member')->check() )
                    {
                        ...
                        $members = Member::when($request->health_status, function ($query, $health_status) {
                            return $query->where('health_status', 'like', "%{$health_status}%");
                        })->where('id', '!=', $auth_id)->where('gender', 'ذكر')->orderBy('id', 'desc')->paginate(14);
                    }else{
                        $members = Member::when($request->health_status, function ($query, $health_status) {
                            return $query->where('health_status', 'like', "%{$health_status}%");
                        })->where('gender', 'ذكر')->orderBy('id', 'desc')->paginate(14);
                    }
                    $members_counts = count($members);
                    $title = 'ذكر';
                    return view('includes.sofia_front.health_members', compact('members', 'members_counts', 'title'));
                ...
                case 'أنثي':
                    if( Auth::guard('member')->check() )
                    {
                        ...
                        $members = Member::when($request->health_status, function ($query, $health_status) {
                            return $query->where('health_status', 'like', "%{$health_status}%");
                        })->where('id', '!=', $auth_id)->where('gender', 'أنثي')->orderBy('id', 'desc')->paginate(14);
                    }else{
                        $members = Member::when($request->health_status, function ($query, $health_status) {
                            return $query->where('health_status', 'like', "%{$health_status}%");
                        })->where('gender', 'أنثي')->orderBy('id', 'desc')->paginate(14);
                    }
                    $members_counts = count($members);
                    $title = 'أنثي';
                    return view('includes.sofia_front.health_members', compact('members', 'members_counts', 'title'));
